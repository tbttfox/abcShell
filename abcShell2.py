...
def buildBorderConnections(edges, offset, flip):
    """Build the new polygons connecting the inside/outside of the shell along the border

    Arguments:
        edges (np.array): (N*2) Edge index pairs
        offset (int): The numerical offset to give each vertex to find its duplicate
        flip (bool): Whether to reverse the original (True) or the offsetted (False)
            edges to make the new edge loops

    Returns:
        np.array: The number of verts per face
        np.array: The flattened indices of each vert
    """
    # All new faces will have a count of 4
    c = np.ones(len(edges), dtype=int) * 4

    edges = np.copy(edges).reshape((-1, 2))
    ne = np.copy(edges) + offset
    if flip:
        edges = np.flip(edges, axis=1)
    else:
        ne = np.flip(ne, axis=1)

    f = np.concatenate((edges, ne), axis=1).flatten()
    return c, f
